...
use crate::io::{write_csv_policemen_to_file, write_csv_vehicle_to_file, write_database_policemen_to_file, write_database_vehicle_to_file, write_patrols_to_file, write_places_to_file, write_policeman_patrol_to_file, write_reports_to_file};
...
fn main() {
    ...
    let mut policemen = (0..POLICEMEN_COUNT)
        .map(|index| Policeman::generate_just_employed_with_id(&mut generator, &data_start_date, index))
        .collect::<Vec<_>>();
    ...
    events.push(Event { time: data_start_date, action: EventAction::Report });
    ...
    loop {
        ...
        match event.action {
            EventAction::PolicemanEmployment => {
                let policeman = Policeman::generate_just_employed_with_id(&mut generator, &current_time, policemen.len());
                policemen.push(policeman);
                let event = Event::from_policeman_resignation_event(&policeman);
                events.push(event);
            }
            EventAction::PolicemanResignation => {
                let next_policeman_employment_date = current_time.checked_add_days(Days::new(7)).unwrap();
                let event = Event { time: next_policeman_employment_date, action: EventAction::PolicemanEmployment };
                events.push(event);
            }
            EventAction::Report => {
                let report_id = reports.len();
                let report = Report::generate_with_time_and_id(&mut generator, current_time, places.len(), reports.len());
                reports.push(report);
                ...
                let required_patrols = if generator.gen_bool(TWO_PATROLS_CHANCE) { 2 } else { 1 };
                for _ in 0..required_patrols {
                    events.push(Event { time: next_report_time, action: EventAction::SendPatrol(report_id) });
                }

                let time_between_reports = generator.gen_range(MIN_SECONDS_BETWEEN_REPORTS..MAX_SECONDS_BETWEEN_REPORTS);
                let next_report_time = current_time.checked_add_signed(Duration::seconds(time_between_reports)).unwrap();
                let event = Event { time: next_report_time, action: EventAction::Report };
                events.push(event);
            }
            EventAction::SendPatrol(report_id) => {
                ...
                let patrol = Patrol::generate_with_report_id_policemen_vehicles_and_sending_time_and_id(&mut generator, report_id, policemen_ids, vehicle_id, current_time, patrol_id);

                events.push(Event { time: patrol.finish_time, action: EventAction::FinishedPatrol(patrol_id) });
                patrols.push(patrol);
            }
            ...
            EventAction::Snapshot(snapshot_name, is_terminal) => {
                ...
                dbg!(snapshot_name, is_terminal, places.len(), policemen.len(), vehicles.len(), reports.len(), patrols.len());
